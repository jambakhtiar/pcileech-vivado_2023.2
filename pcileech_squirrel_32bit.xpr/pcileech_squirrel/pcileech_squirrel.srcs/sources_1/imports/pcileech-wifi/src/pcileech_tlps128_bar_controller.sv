...
module pcileech_tlps128_bar_controller(
    ...
    pcileech_bar_impl_dvb_adapter i_bar0(
        .rst                   ( rst                           ),
        .clk                   ( clk                           ),
        .wr_addr               ( wr_addr                       ),
        .wr_be                 ( wr_be                         ),
...
// ------------------------------------------------------------------------
// dvb_adapter BAR implementation
// ------------------------------------------------------------------------
/*
module pcileech_bar_impl_dvb_adapter_32bit(
    input               rst,
    input               clk,
    // incoming BAR writes:
    input [31:0]        wr_addr,
    ...
    reg [31:0]      lnb_power_reg;
    reg [31:0]      modulation_reg;
    reg [31:0]      symbol_rate_reg;
    reg [31:0]      frequency_reg;
    reg [31:0]      version_reg;
    reg [31:0]      interrupt_status_reg;
    reg [31:0]      interrupt_enable_reg;
    reg [31:0]      serial_number_reg_high;
    reg [31:0]      serial_number_reg_low;
    reg [31:0]      temperature_reg;
    reg [31:0]      fan_speed_reg;
    ...
    always @ (posedge clk) begin
        if (rst) begin
            ...
            serial_number_reg_high  <= 32'h00000231; // Serial number high part: 2313
            serial_number_reg_low   <= 32'h02200020; // Serial number low part: 02200020
            ...
            if (drd_req_valid) begin
                case (drd_req_addr - base_address_register)
                    ...
                    16'h0024 : rd_rsp_data <= serial_number_reg_high;  // Serial Number Register (High)
                    16'h0028 : rd_rsp_data <= serial_number_reg_low;   // Serial Number Register (Low)
                    16'h002C : rd_rsp_data <= temperature_reg;         // Temperature Register
                    16'h0030 : rd_rsp_data <= fan_speed_reg;           // Fan Speed Register
                    default  : rd_rsp_data <= 32'hDEADBEEF;            // Default value: 0xDEADBEEF
                endcase
                ...
                case (dwr_addr - base_address_register)
                    ...
                    16'h0024 : serial_number_reg_high <= dwr_data;     // Write to Serial Number Register (High)
                    16'h0028 : serial_number_reg_low <= dwr_data;      // Write to Serial Number Register (Low)
                    16'h002C : temperature_reg <= dwr_data;            // Write to Temperature Register
                    16'h0030 : fan_speed_reg <= dwr_data;              // Write to Fan Speed Register
...
module pcileech_bar_impl_dvb_adapter(
    ...
    reg [31:0]      version_reg;
    reg [31:0]      interrupt_status_reg;
    reg [31:32]     interrupt_enable_reg;
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (rd_req_valid) begin
                ...
                rd_rsp_valid <= 1;
                case (rd_req_addr - base_address_register)
                    16'h0000: rd_rsp_data <= status_reg;            // Status Register
                    16'h0004: rd_rsp_data <= control_reg;           // Control Register
                    16'h0008: rd_rsp_data <= version_reg;           // Version Register
                    16'h000C: rd_rsp_data <= lnb_power_reg;         // LNB Power Register
            ...
            if (wr_valid) begin
                case (wr_addr - base_address_register)
                    16'h0004: control_reg <= wr_data;              // Write to Control Register
                    16'h000C: lnb_power_reg <= wr_data;            // Write to LNB Power Register
                    16'h0010: modulation_reg <= wr_data;           // Write to Modulation Register
                    16'h0014: symbol_rate_reg <= wr_data;          // Write to Symbol Rate Register
                    16'h0018: frequency_reg <= wr_data;            // Write to Frequency Register
                    16'h0020: interrupt_enable_reg <= wr_data;     // Write to Interrupt Enable Register
                    default : ; // No action for other addresses
                endcase
            end
        end
